...
return [
     ...
     */
    // Only the public/integration-facing routes are documented here — the
    // dashboard routes (auth:sanctum) are internal to the Klea frontend, not
    // something third-party integrators call directly. The Semoa callback is
    // also excluded: it's called by the payment provider server-to-server,
    // verified by JWT signature rather than an API key, and never something
    // an integrating developer calls themselves.
    'api_path' => [
        'include' => 'api/public',
        'exclude' => ['api/public/semoa*'],
    ],
    ...
    'info' => [
         ...
         */
        'description' => 'Endpoints for integrating Klea subscriptions into your own app. '
            .'Authenticate with an API key from your Klea dashboard: Authorization: Bearer {public_id}.{secret}.',
    ],
     ...
     * ],
     */
    // Public routes use a Klea API key (Bearer {public_id}.{secret} via the
    // `api.key` middleware), not Sanctum — so we target that middleware
    // specifically rather than the default `auth`/`auth:*` patterns.
    'security_strategy' => [
        \Dedoc\Scramble\SecurityDocumentation\MiddlewareAuthSecurityStrategy::class,
        [
            'middleware' => ['api.key'],
